...
const NewBooking: React.FC<NewBookingProps> = ({ user, products, onBookingSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await notificationService.sendBookingEmail(newBooking, user);
      
      // 2. Trigger WhatsApp (Optional)
      await notificationService.sendWhatsAppAlert(newBooking, user);

      // Simulate network delay for DB write
      setTimeout(() => {
      ...
      setIsSubmitting(false);
      alert('Order placed but notification failed. Admin is still notified via Dashboard.');
    }
  };
    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-2">
          ...
          >
            {isSubmitting ? 'Sending Notifications...' : 'Place Order'}
          </button>
        </div>
      </form>
    </div>
