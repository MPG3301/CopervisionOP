...
const NewBooking: React.FC<NewBookingProps> = ({ user, products, onBookingSubmit }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.size > 2 * 1024 * 1024) {
      alert("Error: File exceeds 2MB limit. Please select a smaller file.");
      e.target.value = '';
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!productId) return alert('Please select a product model');
    ...
    try {
      if (billFile) {
        ...
        const fileName = `bills/${user.id}_${Date.now()}.${fileExt}`;
  ...
  return (
    <div className="p-6 space-y-6 animate-in fade-in duration-500">
      <header>
        <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">New Dispatch</h2>
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Submit sale details for verification</p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white p-7 rounded-[40px] border border-slate-100 shadow-sm space-y-7">
          ...
          <div className="space-y-2 pt-2 border-t border-slate-50 mt-4">
            ...
            <div className="relative group overflow-hidden rounded-2xl">
              <input type="file" accept="image/jpeg,image/png,application/pdf" onChange={handleFileChange} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
              <div className={`w-full py-8 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all ${billFile ? 'bg-blue-50 border-[#005696]' : 'bg-slate-50 border-slate-200 group-hover:border-[#005696]'}`}>
                ...
                <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest text-center px-4 leading-relaxed">
                  {billFile ? billFile.name : 'Tap to select bill image/pdf'}
                </span>
                {billFile && <span className="text-[8px] font-black text-[#005696] uppercase mt-2">File Ready for upload</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#005696] p-7 rounded-[40px] text-white text-center shadow-2xl shadow-blue-900/20 relative overflow-hidden group">
           <div className="absolute top-0 right-0 p-4 opacity-5 rotate-12 group-hover:scale-110 transition-transform"><ICONS.Rewards className="w-24 h-24" /></div>
        ...
        <button 
          ...
          className={`w-full py-7 rounded-[30px] font-black text-xl text-white shadow-2xl transition-all active:scale-95 ${isSubmitting ? 'bg-slate-300' : 'bg-slate-900 shadow-slate-900/30'}`}
        >
          {isSubmitting ? 'Verifying...' : 'SUBMIT TO WAITLIST'}
        </button>
      </form>
    </div>
  );
