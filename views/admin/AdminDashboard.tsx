...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      // Expected CSV format: Name, Brand, Points, Price
        ...
        <header className="bg-white px-6 py-5 border-b flex justify-between items-center z-30 shadow-sm lg:hidden shrink-0">
           <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest leading-none mt-0.5">{view.toUpperCase()}</span>
              </div>
           </div>
          ...
          {view === 'overview' && (
            <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
              <header>
                ...
                <p className="text-slate-400 font-bold uppercase text-[9px] tracking-[0.2em] mt-1">Real-time business performance</p>
              </header>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              <header className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-1 italic">Verify optometrist claims</p>
                </div>
              </header>
              <div className="space-y-4">
                {props.bookings.filter(b => b.status === 'waiting').length === 0 ? (
                   <div className="py-24 text-center text-slate-300 font-black italic text-xs uppercase border-2 border-dashed border-slate-100 rounded-[30px]">All caught up</div>
                ) : (
                    ...
                    <div key={b.id} className="bg-white p-5 rounded-[30px] border border-slate-100 shadow-sm flex flex-col gap-4">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">CLAIM BY: {b.optometrist_name || 'Partner'}</p>
                        </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => handleUpdateBooking(b.id, 'approved')} className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg shadow-emerald-100">Confirm</button>
                        <button onClick={() => handleUpdateBooking(b.id, 'rejected')} className="flex-1 py-3 bg-slate-100 text-slate-400 rounded-xl text-[9px] font-black uppercase">Deny</button>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="max-w-5xl mx-auto space-y-8">
              <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                   ...
                   <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-1 italic">Live rewards inventory</p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <button 
                    ...
                    className="flex-1 sm:flex-none px-6 py-3.5 bg-slate-800 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-slate-900/20"
                  >
                    {isBulkUploading ? 'Processing...' : 'Bulk Import'}
                  </button>
                  <button onClick={() => setEditingProduct({} as any)} className="p-3.5 bg-[#005696] text-white rounded-xl shadow-lg shadow-blue-900/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {props.products.map(p => (
                  <div key={p.id} className={`bg-white p-6 rounded-[35px] border border-slate-100 shadow-sm flex flex-col justify-between group transition-all ${!p.active ? 'opacity-50 grayscale' : ''}`}>
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-[8px] font-black text-[#005696] uppercase tracking-[0.2em]">{p.brand}</p>
                        <h4 className="text-sm font-black text-slate-900 mt-1 leading-tight">{p.product_name}</h4>
                      </div>
                      <div className={`w-3 h-3 rounded-full ${p.active ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : 'bg-slate-300'}`}></div>
                    </div>
                    
                    <div className="flex items-center justify-between bg-slate-50/50 p-3 rounded-2xl mb-4 border border-slate-50">
                       <div className="flex flex-col">
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Rewards</span>
                          <span className="text-xs font-black text-slate-700">{p.points_per_unit} pts</span>
                       </div>
                       <div className="flex flex-col text-right">
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Price</span>
                          <span className="text-xs font-black text-slate-700">₹{p.base_price}</span>
                       </div>
                    </div>

                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setEditingProduct(p)} className="flex-1 py-2 bg-slate-100 text-slate-600 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-slate-200">Edit</button>
                      <button onClick={() => handleToggleProduct(p)} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest text-white ${p.active ? 'bg-amber-500' : 'bg-emerald-500'}`}>
          ...
          {view === 'withdrawals' && (
             <div className="max-w-4xl mx-auto space-y-6">
                <header>
                   ...
                   <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-1 italic">Process cash payouts</p>
                </header>
                <div className="space-y-4">
                   {props.withdrawals.length === 0 ? (
                      ...
                      props.withdrawals.map(w => (
                         <div key={w.id} className="bg-white p-6 rounded-[30px] border border-slate-100 shadow-sm flex flex-col gap-5">
                            <div className="flex justify-between items-center">
                               <div className="flex items-center gap-4">
                                  <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-[#005696]">
                                     <ICONS.Rewards className="w-6 h-6" />
                                  </div>
                                  <div>
                                     <p className="text-xl font-black text-slate-900 leading-none">₹{w.amount}</p>
                                     <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{w.upi_id}</p>
                                  </div>
                               </div>
                               <span className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest ${w.status === 'approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{w.status}</span>
                            </div>
                            {w.status === 'pending' && (
                               <div className="flex gap-2 pt-2 border-t border-slate-50">
                                  <button className="flex-1 py-3.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-slate-900/30 active:scale-95 transition-transform">Verify & Pay</button>
                                  <button className="flex-1 py-3.5 bg-slate-50 text-slate-300 rounded-xl text-[10px] font-black uppercase tracking-widest">Ignore</button>
                               </div>
                            )}
                         </div>
                      ))
      ...
      {editingProduct && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-lg rounded-[40px] p-8 shadow-2xl relative">
              <button onClick={() => setEditingProduct(null)} className="absolute top-6 right-6 p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:text-red-500 transition-colors"><ICONS.X className="w-5 h-5"/></button>
              ...
              <form onSubmit={async (e) => {
                ...
                if (!res.error && res.data) {
                 ...
                 <Input label="Manufacturer" name="pbrand" defaultValue={editingProduct.brand || "CooperVision"} />
                 <Input label="Display Title" name="pname" defaultValue={editingProduct.product_name} required />
                 <div className="grid grid-cols-2 gap-4">
                   <Input label="Base Price (₹)" name="pprice" type="number" defaultValue={editingProduct.base_price} required />
                   <Input label="Points Awarded" name="ppoints" type="number" defaultValue={editingProduct.points_per_unit} required />
                 </div>
                 <button type="submit" className="w-full py-5 bg-[#005696] text-white rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-2xl shadow-blue-900/30 mt-4 active:scale-95 transition-transform">Commit to Catalog</button>
              </form>
           </div>
        </div>
      )}
...
const AdminNavItem = ({ active, label, icon, onClick }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1.5 transition-all ${active ? 'text-[#005696] scale-110' : 'text-slate-300'}`}>
    ...
    <span className={`text-[8px] font-black uppercase tracking-wider text-center ${active ? 'text-[#005696]' : 'text-slate-300'}`}>{label}</span>
  </button>
);
...
const Input = ({ label, ...props }: any) => (
  <div className="space-y-1">
    ...
    <input {...props} className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl outline-none focus:border-[#005696] focus:bg-white font-bold text-slate-800 transition-all text-sm" />
  </div>
);
